...
/*document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchComments() {
        ...
        try {
            ...
            setupCommentsEventListener(); // Attach event listeners to comment actions
        } catch (error) {
            console.error('Error fetching comments:', error);
        }
    }

    // Set up event listeners for comment actions (edit, delete)
    function setupCommentsEventListener() {
        const commentsList = document.getElementById('comments-list');
        commentsList.removeEventListener('click', handleCommentActions);
        commentsList.addEventListener('click', handleCommentActions);
        ...
        commentForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    editingCommentId = null;
                    submitButton.textContent = '댓글 작성';
                    document.getElementById('comment-username').removeAttribute('disabled');
                } else {
                    alert('댓글 작성/수정 실패');
                }
            } catch (error) {
    ...
    function startEditComment(commentElement) {
        ...
        const commentUsername = commentElement.querySelector('strong').innerText;

        // Fill the form with existing comment data
        const usernameInput = document.getElementById('comment-username');
        const contentInput = document.getElementById('comment-content');
        const submitButton = document.querySelector('#comment-form button');

        usernameInput.value = commentUsername;
        usernameInput.setAttribute('disabled', 'true'); // Disable username field
    ...
    // Fetch board details when the page loads
    fetchBoardDetails();
});*/
